...
Identiy uses prebuilt managers (namely ApplicationUserManager). This caused issues when used in concert with the services I created. User objects retreived through an applicationManager could not have a band added to them as they used different dbContexts. Updating the dbContext in the ApplicationUserManager caused issues. In the interest of time, I create a very simple user service to retrieve user objects from the same context.
